...
import {cdp,viewport,navigate,revealAll,screenshot,section,evaluate,pause,close,browserErrors} from './cdp.mjs';
const checks=[];
...
await pointer('[data-city="Warszawa"] .map-city__pin');
...
await pointer('[data-city="Warszawa"] .map-city__pin','click');
...
await pointer('[data-city="Poznań"] .map-city__pin');
...
check('Arrow key moves map focus',await evaluate("document.activeElement.matches('.map-city') && document.activeElement.dataset.city!=='Katowice'"));
await key(' ','Space',32);
...
for(const [width,height] of [[1440,900],[1280,800],[768,1024],[390,844]]){
 ...
 const hitFailures=await evaluate(`(()=>[...document.querySelectorAll('.map-city')].flatMap(city=>{const r=city.querySelector('.map-city__pin').getBoundingClientRect();const hit=document.elementFromPoint(r.x+r.width/2,r.y+r.height/2)?.closest('.map-city');return hit===city?[]:[{city:city.dataset.city,hit:hit?.dataset.city}]}))()`);
 ...
 if(width===390){
  await pointer('[data-city="Warszawa"] .map-city__pin','tap');
...
const reduced=await evaluate("({matches:matchMedia('(prefers-reduced-motion: reduce)').matches,hidden:[...document.querySelectorAll('[data-reveal],.map-city,.map__boundaries')].filter(e=>getComputedStyle(e).opacity!=='1').length,smooth:getComputedStyle(document.documentElement).scrollBehavior})");
...
for(const [width,height] of [[1440,900],[390,844]]){
 await viewport(width,height,width===390);await navigate();
 await evaluate(`(async()=>{const img=new Image();img.className='hero__photo';img.alt='';img.src='qa/photo-fixture.jpg';document.querySelector('.hero').prepend(img);await img.decode()})()`);await pause(200);
 check(`${width}: photo hero loads and stays within viewport`,await evaluate("document.querySelector('.hero__photo').naturalWidth>0 && document.documentElement.scrollWidth<=innerWidth && getComputedStyle(document.querySelector('.hero'),'::before').backgroundColor==='rgba(12, 31, 77, 0.82)'"));
...
await writeFile('qa/results.json',JSON.stringify({browser:'Chrome 154',date:'2026-09-20',checks},null,2));
close();
