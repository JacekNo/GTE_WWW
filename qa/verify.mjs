...
// scrollbar, which changes map hit coordinates until the next input repaint.
...
check('Map hover label',await evaluate("document.querySelector('[data-map-tooltip-label]').textContent==='Warszawa' && document.querySelector('[data-map-tooltip]').classList.contains('is-visible')"));
await cdp('Input.dispatchMouseEvent',{type:'mouseMoved',x:10,y:100});await pause(100);
check('Map tooltip hides after pointer leaves',await evaluate("!document.querySelector('[data-map-tooltip]').classList.contains('is-visible') && !document.querySelector('.map-city.is-highlighted')"));
await pointer('[data-city="Poznań"]');
check('Hovering another city shows only transient state',await evaluate("document.querySelector('[data-map-tooltip-label]').textContent==='Poznań' && document.querySelectorAll('.map-city.is-highlighted').length===1"));
await cdp('Input.dispatchMouseEvent',{type:'mouseMoved',x:10,y:100});await pause(100);
check('Map pins are informational, not buttons',await evaluate("document.querySelector('.map-city').tagName==='SPAN' && !document.querySelector('.map-city').hasAttribute('tabindex') && !document.querySelector('.map-city').hasAttribute('aria-pressed') && document.querySelector('[data-map-pins]').getAttribute('aria-hidden')==='true'"));
for(const [width,height] of [[1440,900],[1280,800],[768,1024],[390,844]]){
 ...
 const pinBounds=await evaluate(`(()=>{const stage=document.querySelector('[data-map-stage]').getBoundingClientRect();return [...document.querySelectorAll('.map-city')].flatMap(city=>{const r=city.getBoundingClientRect();const inside=r.left>=stage.left-1&&r.right<=stage.right+1&&r.top>=stage.top-1&&r.bottom<=stage.bottom+1;return inside?[]:[{city:city.dataset.city,left:r.left,top:r.top,right:r.right,bottom:r.bottom}];});})()`);
 check(`${width}: all 46 map pins stay inside map bounds`,pinBounds.length===0,pinBounds);
 const edges=await evaluate(`(async()=>{const bad=[];const map=document.querySelector('[data-map]');const select=map.querySelector('[data-map-picker]');for(const city of map.querySelectorAll('.map-city')){select.value=city.dataset.city;select.dispatchEvent(new Event('change',{bubbles:true}));await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));const r=map.getBoundingClientRect(),t=map.querySelector('[data-map-tooltip]').getBoundingClientRect();if(t.left<r.left-1||t.right>r.right+1||t.top<r.top-1)bad.push(city.dataset.city);}select.value='';select.dispatchEvent(new Event('change',{bubbles:true}));return bad;})()`);
 check(`${width}: all 46 tooltip edge positions`,edges.length===0,edges);
 if(width===390){
  await pointer('[data-city="Warszawa"]','tap');
  check('Mobile pin tap has no action',await evaluate("!document.querySelector('[data-city=Warszawa]').classList.contains('is-picker-selected') && !document.querySelector('[data-map-tooltip]').classList.contains('is-visible')"));
  ...
  check('Mobile list highlights Silesia city',await evaluate("document.querySelector('[data-city=Katowice]').classList.contains('is-picker-selected') && document.querySelectorAll('.is-picker-selected').length===1 && document.querySelector('[data-map-tooltip-label]').textContent==='Katowice'"));
...
for(const [width,height] of [[1440,900],[390,844]]){
 ...
 check(`${width}: optimized photo hero loads and stays within viewport`,await evaluate("performance.getEntriesByType('resource').some(entry=>entry.name.includes('/assets/photos/hero-gte.avif')) && document.documentElement.scrollWidth<=innerWidth && getComputedStyle(document.querySelector('.hero')).backgroundImage!=='none' && getComputedStyle(document.querySelector('.hero'),'::before').backgroundImage!=='none'"));
